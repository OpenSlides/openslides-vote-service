...
func TestRatingApprovalCreateResult(t *testing.T) {
	for _, tt := range []struct {
	...
	}{
		{
			...
			expectResult: `{"1":{"yes":"1"},"2":{"no":"1","yes":"1"},"3":{"no":"1","yes":"5"},"turnout":3}`,
		},
		{
			...
			expectResult: `{"1":{"yes":"1"},"2":{"no":"1"},"invalid":1,"turnout":2}`,
		},
		{
			...
			expectResult: `{"1":{"yes":"1"},"2":{"no":"1"},"abstain":"1","turnout":2}`,
		},
		{
			...
			expectResult: `{"1":{"yes":"1"},"2":{"no":"1"},"abstain":"1","invalid":1,"turnout":3}`,
		},
		{
			...
			expectResult: `{"1":{"yes":"2"},"2":{"abstain":"1"},"turnout":2}`,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
			a, err := method.RatingApprovalFromRequest([]byte(tt.config))
