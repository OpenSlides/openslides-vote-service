...
CATCH=0
DOCKER_EXEC="docker exec vote-test"
...
then
    # Container Mode
    if [ -z "$SKIP_CONTAINER_UP" ]; then docker run -d -t --name vote-test ${IMAGE_TAG} || CATCH=1; fi
...
if [ -z "$PERSIST_CONTAINERS" ] && [ -z "$SKIP_CONTAINER_UP" ]; then docker stop vote-test && docker rm vote-test || CATCH=1; fi
